...
use crate::modules::hashtag::service::HashtagService;
use crate::modules::users::repository::UserRepository;
use crate::notifications::NotificationService;
use crate::websocket::{WebSocketServer, WsMessage};

use super::models::{
    CreateCommentRequest, CreatePostRequest, CreateRepostRequest, FeedSort, TimelineFeedQuery,
    UpdatePostRequest,
};
...
pub async fn create_post(
    ...
    hashtag_service: web::Data<HashtagService>,
    ...
    let _ = hashtag_service.process_post_hashtags(post.id, &content).await;
...
pub async fn get_following_feed(
    ...
    if offset == 0 {
        if let Some(ref cache) = cache {
            if let Ok(Some(posts)) = cache
                .get::<Vec<super::models::PostResponse>>(&cache_key)
                .await
            {
                return Ok(HttpResponse::Ok().json(ApiResponse::success(posts)));
            }
    ...
    let posts = service
        .get_following_feed(user.user_id, offset, limit, sort)
        .await?;
    ...
    if offset == 0 {
        if let Some(cache) = cache {
            let _ = cache
                .set(&cache_key, &posts, std::time::Duration::from_secs(120))
                .await;
        }
    }
...
pub async fn like_post(
    ...
    if post.author_id != user.user_id {
        ...
        let liker_name = liker_profile.display_name.unwrap_or(liker_profile.username);
...
pub async fn add_comment(
    ...
    if post.author_id != author_id {
        ...
        let commenter_name = commenter_profile
            .display_name
...
pub async fn create_repost(
    ...
    let repost = service
        .create_repost(user.user_id, request.into_inner())
        .await?;
...
pub async fn get_my_reposts(
    ...
    let reposts = service
        .get_user_reposts(user.user_id, offset, limit)
        .await?;
...
pub async fn get_for_you_feed(
    ...
    let posts = service
        .get_for_you_feed(user.user_id, offset, limit)
        .await?;
...
pub async fn get_trending_posts(
    ...
    let posts = service
        .get_trending_posts(user.user_id, offset, limit)
        .await?;
    Ok(HttpResponse::Ok().json(ApiResponse::success(posts)))
}
