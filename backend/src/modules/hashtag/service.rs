...
impl HashtagService {
    ...
    #[allow(dead_code)]
    pub async fn process_post_hashtags(&self, post_id: uuid::Uuid, content: &str) -> Result<(), AppError> {
        let hashtags = extract_hashtags(content);
        
        for tag in hashtags {
            let hashtag = self.repository.find_or_create(&tag).await?;
            self.repository.link_hashtag_to_post(post_id, hashtag.id).await?;
        }
    ...
    #[allow(dead_code)]
    pub async fn remove_post_hashtags(&self, post_id: uuid::Uuid, content: &str) -> Result<(), AppError> {
        let hashtags = extract_hashtags(content);
        
        for tag in hashtags {
            if let Some(hashtag) = self.repository.find_by_name(&tag).await? {
                self.repository.unlink_hashtag_from_post(post_id, hashtag.id).await?;
            }
        }
    ...
    #[allow(dead_code)]
    pub async fn get_post_hashtags(&self, post_id: uuid::Uuid) -> Result<Vec<HashtagResponse>, AppError> {
